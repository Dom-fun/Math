function circletriangleMain() {
  ...
  s += '<input id="degBtn" type="button" class="togglebtn hi"  style="z-index:2; position:absolute; top:445px; left:310px; width:95px;" value="弧度" onclick="toggleDeg()"/>';
  s += '<input id="coordBtn" type="button" class="togglebtn lo"  style="z-index:2; position:absolute; top:445px; left:10px; width:95px;" value="坐标" onclick="toggleCoord()"/>';
  s += '<input id="quadBtn" type="button" class="togglebtn lo"  style="z-index:2; position:absolute; top:445px; left:110px; width:95px;" value="象限"  onclick="toggleQuad()"/>';
  s += '<input id="angBtn" type="button" class="togglebtn lo"  style="z-index:2; position:absolute; top:445px; left:210px; width:95px;" value="角度" onclick="toggleAngles()"/>';
  s += '</div>';
  document.write(s);
...
function toggleDeg() {
  if (degQ) {
    degQ = false;
    document.getElementById("degBtn").value = "弧度"
  } else {
    degQ = true;
    document.getElementById("degBtn").value = "角度"
  }
  update();
}
...
function drawAngle() {
  ...
  g.textAlign = "center";
  g.fillStyle = sinClr;
  g.fillText("sin(" + angDisplay + ") = " + fs[0], w / 2, 20, w);
  g.fillStyle = cosClr;
  g.fillText("cos(" + angDisplay + ") = " + fs[1], w / 2, 45, w);
  g.fillStyle = tanClr;
